...
.prechat-form-head{font-size: 0.75rem; font-weight: 300; color: var(--muted);}
  ...
  `,o=document.createElement("style");o.type="text/css",o.appendChild(document.createTextNode(e)),document.head.appendChild(o)}function oe(e){e&&document.documentElement.style.setProperty("--brand",e)}var q=class{constructor(t,n,o){this.root=t,this.routes=n||{},this.currentPage="welcome",this.onRendered=o,localStorage.setItem("chatbot_current_page",this.currentPage)}getPage(){return this.currentPage}setPage(t){this.currentPage=t,this.render()}render(){let t=this.getPage(),n=this.routes[t]||this.routes.welcome;this.root.innerHTML=typeof n=="function"?n():n,typeof this.onRendered=="function"&&this.onRendered(this),localStorage.setItem("chatbot_current_page",this.currentPage)}};function ne(e="Zentrova"){return{welcome:()=>`
    ...
    `}}function ke(e){try{if(!e||e.length!==2)return"";let t=[...e.toUpperCase()].map(n=>127397+n.charCodeAt(0));return String.fromCodePoint(...t)}catch{return""}}async function j(e,t="+234"){let n=document.getElementById(e);if(n){try{n.innerHTML='<option value="">Loading countries...</option>'}catch{}try{let o=await fetch("https://restcountries.com/v3.1/all?fields=name,cca2,idd");if(!o.ok)throw new Error("Failed to fetch countries");let c=await o.json(),s=[];for(let i of c){let p=i?.name?.common||"",r=i?.cca2||"",d=i?.idd?.root||"",u=Array.isArray(i?.idd?.suffixes)?i.idd.suffixes:[];if(!p||!d||u.length===0)continue;let _=ke(r);for(let h of u){let x=`${d}${h}`;s.push({name:p,dial:x,flag:_})}}s.sort((i,p)=>i.name.localeCompare(p.name));let a="";for(let i of s){let p=i.dial===t?" selected":"",r=`${i.flag?i.flag+" ":""}${i.name} (${i.dial})`;a+=`<option value="${i.dial}"${p}>${r}</option>`}if(a)n.innerHTML=a;else throw new Error("No countries with dialing codes")}catch(o){console.warn("Country population failed:",o?.message||o);try{(!n.innerHTML||n.innerHTML.includes("Loading countries"))&&(n.innerHTML=['<option value="+234">\u{1F1F3}\u{1F1EC} Nigeria (+234)</option>','<option value="+1">\u{1F1FA}\u{1F1F8} United States (+1)</option>','<option value="+44">\u{1F1EC}\u{1F1E7} United Kingdom (+44)</option>'].join(""))}catch{}}}}var G="https://api.zentai.cloud/api/v1",ae="https://chatbot.zentai.cloud/",H=null,K=null,z=null,V=null,Y=null,Ee=!1,N=!1,W=!1,U=null,C=null,I=null,T=null,M=null,P=null,R=null,O=null,L=null,$=Date.now(),A=null,X=!1;function F(){$=Date.now()}function ce(){try{W=!0,V&&(V.disconnect(),V=null),z&&(z.disconnect(),z.mediaStream&&z.mediaStream.getTracks().forEach(e=>e.stop()),z=null);try{U&&(U.stop(),U.disconnect())}catch{}if(U=null,Ie=[],Ce=!1,Be=0,K){try{K.close()}catch{}K=null}if(H){try{H.readyState===WebSocket.OPEN&&H.close(1e3,"User ended call")}catch{}H=null}Y&&(clearInterval(Y),Y=null);try{if(I){try{I.close()}catch{}I=null}if(C){try{C.close()}catch{}C=null}if(T){try{T.getTracks().forEach(t=>t.stop())}catch{}T=null}let e=document.getElementById("assistantAudio");e&&(e.srcObject=null),O&&(cancelAnimationFrame(O),O=null);try{P&&P.disconnect()}catch{}P=null;try{R&&R.disconnect()}catch{}if(R=null,M){try{M.close()}catch{}M=null}L!=null&&(clearInterval(L),L=null),X=!1}catch{}Ee=!1,N=!1}catch{}}function Se(){let e=localStorage.getItem("chatbot_session_id");return e||(e="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(t){let n=Math.random()*16|0;return(t==="x"?n:n&3|8).toString(16)}),localStorage.setItem("chatbot_session_id",e)),e}function D(e,t={}){try{window.dispatchEvent(new CustomEvent("chatbot-call-status",{detail:{status:e,...t}}))}catch{}}var Ie=[],Ce=!1,Be=0;function re(e,t){let n=e.getPage();if(n==="call-welcome"){let o=document.getElementById("call-welcome-back"),c=document.getElementById("call-welcome-start");o&&o.addEventListener("click",()=>e.setPage("welcome")),c&&c.addEventListener("click",()=>e.setPage("precall"));let s=document.getElementById("footer-call"),a=document.getElementById("footer-chat");s&&s.classList.add("active"),a&&a.classList.remove("active"),a&&a.addEventListener("click",()=>e.setPage("prechat"));return}if(n==="precall"){let d=function(){let h=(a&&a.value||"").trim(),k=(p&&p.value||"").trim().length>=7,f=!!(r&&r.checked)},u=function(h){c&&(c.disabled=!!h);let x=document.querySelector(".precall-card");x&&(x.style.pointerEvents=h?"none":"auto"),x&&(x.style.opacity=h?"0.6":"1")},o=document.getElementById("precall-form"),c=document.getElementById("precall-start"),s=document.getElementById("precall-back"),a=document.getElementById("precall-name"),i=document.getElementById("precall-country"),p=document.getElementById("precall-phone"),r=document.getElementById("precall-consent");j("precall-country","+234"),d(),a&&a.addEventListener("input",d),p&&p.addEventListener("input",d),i&&i.addEventListener("change",d),r&&r.addEventListener("change",d);async function _(){if(N)return;N=!0,u(!0);let h=(document.getElementById("precall-name").value||"Guest").trim(),x=document.getElementById("precall-country").value||"",k=(document.getElementById("precall-phone").value||"").trim(),f=localStorage.getItem("chatbot_user_question")||"Start call";try{localStorage.setItem("chatbot_user_name",h),localStorage.setItem("chatbot_user_phone",`${x}${k}`),localStorage.setItem("chatbot_user_question",f)}catch{}let b=t&&t.chatbotID||localStorage.getItem("chatbot_id");if(!b){console.error("Missing chatbotID"),N=!1,u(!1);return}try{let l=await fetch(`${G}/chatbots/call/${encodeURIComponent(b)}/start/`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({user_phone:`${x}${k}`,user_name:h??"Guest"})});if(!l.ok)throw new Error("Call init failed");let m=await l.json();try{let S=m&&(m.call_id||m.conversation_id||m.id);S&&localStorage.setItem("chatbot_conversation_id",String(S))}catch{}let g=t&&t.bot,w=b,y=g&&g.language||localStorage.getItem("chatbot_bot_language")||"English",E=g&&g.voice||localStorage.getItem("chatbot_bot_voice")||"alloy",v=g&&g.welcome_message||localStorage.getItem("chatbot_bot_welcome_message")||"Hello! How can I assist you today?";await new Promise(S=>setTimeout(S,200)),Ae({agentId:w,language:y,voice:E,welcomeMessage:v}),e.setPage("call")}catch(l){console.error("Init call error:",l),D("error",{error:l?.message||"Initialization failed"}),alert("Unable to start call. Please try again.")}finally{N=!1,u(!1)}}o&&o.addEventListener("submit",h=>{h.preventDefault(),!(c&&c.disabled)&&_()}),c&&c.addEventListener("click",h=>{h.preventDefault(),!(c&&c.disabled)&&_()}),s&&s.addEventListener("click",()=>e.setPage("call-welcome"));return}if(n==="call"){let a=function(p){let d=(p.detail||{}).status,u=document.getElementById("call-connecting");u&&(d==="connected"||d==="configured"?u.style.display="none":d==="connecting"&&(u.style.display="block"))},o=document.getElementById("call-back"),c=document.getElementById("call-end"),s=document.getElementById("call-connecting");s&&(s.style.display="block"),window.addEventListener("chatbot-call-status",a);async function i(){try{let r=localStorage.getItem("chatbot_conversation_id");if(r){await fetch(`${G}/chatbots/call/${encodeURIComponent(r)}/complete/`,{method:"POST",headers:{"Content-Type":"application/json"}});try{localStorage.removeItem("chatbot_conversation_id")}catch{}}else{let d=Se();await fetch(`${G}/chatbots/conversations/${encodeURIComponent(d)}/end/`,{method:"POST",headers:{"Content-Type":"application/json"}})}}catch{}W=!0,ce();let p=null;try{p=localStorage.getItem("chatbot_transfer_phone_number")}catch{}p?(e.setPage("postcall-transfer"),console.log()):e.setPage("call-welcome")}A=i,o&&o.addEventListener("click",p=>{p.preventDefault(),i()}),c&&c.addEventListener("click",p=>{p.preventDefault(),i()});return}if(n==="postcall-transfer"){let o=document.getElementById("transfer-call-now"),c=document.getElementById("transfer-close"),s=null;try{s=localStorage.getItem("chatbot_transfer_phone_number")}catch{}o&&o.addEventListener("click",()=>{if(s)try{window.location.href=`tel:${s}`}catch{}try{localStorage.removeItem("chatbot_transfer_phone_number")}catch{}try{document.body.classList.remove("show-chatbot")}catch{}e.setPage("call-welcome")}),c&&c.addEventListener("click",()=>{try{localStorage.removeItem("chatbot_transfer_phone_number")}catch{}try{document.body.classList.remove("show-chatbot")}catch{}e.setPage("call-welcome")});return}}async function Le(e,t,n,o){let c=`${ae}realtime/token`,s=null;try{let r=localStorage.getItem("chatbot_custom_apis"),d=r?JSON.parse(r):null;s=Array.isArray(d)&&d.length>0?d:null}catch{s=null}let a={};e&&(a.agent_id=e),n&&(a.voice=n),t&&(a.language=t),o&&(a.welcome_message=o),s&&(a.custom_apis=s);let i=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(!i.ok){let r=await i.text();throw new Error(`Failed to obtain token: ${r}`)}let p=await i.json();if(!p.value)throw new Error("Token response missing value field");return p.value}async function Pe(e,t,n){try{let o=JSON.parse(n||"{}");if(o&&o.transfer_phone_number){let a=String(o.transfer_phone_number);try{localStorage.setItem("chatbot_transfer_phone_number",a)}catch{}}let c=await fetch(`${ae}realtime/function-call`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({function_name:e,arguments:o})});if(!c.ok)throw new Error(`Function call failed: ${c.statusText}`);let s=await c.json();if(I&&I.readyState==="open"){let a={type:"conversation.item.create",item:{type:"function_call_output",call_id:t,output:JSON.stringify(s)}};I.send(JSON.stringify(a)),I.send(JSON.stringify({type:"response.create"}))}}catch(o){if(I&&I.readyState==="open"){let c={type:"conversation.item.create",item:{type:"function_call_output",call_id:t,output:JSON.stringify({error:o?.message||"Function call error"})}};I.send(JSON.stringify(c))}}}async function Ae({agentId:e,language:t,voice:n,welcomeMessage:o}){try{let d=function(){X||(X=!0,$=Date.now(),L&&clearInterval(L),L=setInterval(()=>{if(W)return;Date.now()-$>=IDLE_TIMEOUT_MS&&A&&A()},1e3))};D("connecting");let c=document.getElementById("call-connecting");c&&(c.style.display="block");let s=localStorage.getItem("chatbot_id")||e,a=localStorage.getItem("chatbot_bot_voice")||n,i=localStorage.getItem("chatbot_bot_language")||t,p=localStorage.getItem("chatbot_bot_welcome_message")||o,r=await Le(s,i,a,p);C=new RTCPeerConnection({iceServers:[{urls:["stun:stun.l.google.com:19302"]}]}),C.addEventListener("iceconnectionstatechange",()=>{let f=C.iceConnectionState;(f==="disconnected"||f==="failed")&&A&&A()}),T=await navigator.mediaDevices.getUserMedia({audio:!0,video:!1}),T.getTracks().forEach(f=>C.addTrack(f,T)),C.addEventListener("track",f=>{let[b]=f.streams,l=document.getElementById("assistantAudio");l&&(l.srcObject=b);let m=b&&b.getAudioTracks&&b.getAudioTracks()[0];m&&(m.onunmute=()=>{F(),d();let g=document.getElementById("call-connecting");g&&(g.style.display="none"),D("streaming")});try{let g=function(){let w=new Uint8Array(P.frequencyBinCount);P.getByteTimeDomainData(w);let y=0;for(let v=0;v<w.length;v++){let S=(w[v]-128)/128;y+=S*S}Math.sqrt(y/w.length)>.02&&F(),O=requestAnimationFrame(g)};M=new(window.AudioContext||window.webkitAudioContext),R=M.createMediaStreamSource(b),P=M.createAnalyser(),P.fftSize=256,R.connect(P),O=requestAnimationFrame(g)}catch{}}),I=C.createDataChannel("oai-events"),I.onopen=()=>{F(),d();try{I.send(JSON.stringify({type:"response.create"}))}catch{}},I.onmessage=f=>{F(),d();try{let b=JSON.parse(f.data);if(b.type==="response.function_call_arguments.done"){let{name:l,call_id:m,arguments:g}=b;Pe(l,m,g)}}catch{}};let u=await C.createOffer();await C.setLocalDescription(u);let h=await fetch("https://api.openai.com/v1/realtime/calls",{method:"POST",headers:{Authorization:`Bearer ${r}`,"Content-Type":"application/sdp"},body:C.localDescription?.sdp||""});if(!h.ok)throw new Error("SDP exchange failed");let x=await h.text(),k=new RTCSessionDescription({type:"answer",sdp:x});await C.setRemoteDescription(k),L&&clearInterval(L),$=Date.now(),L=setInterval(()=>{if(W)return;Date.now()-$>=1e4&&A&&A()},1e3),D("connected")}catch(c){console.error("LiveCall start error:",c),D("error",{error:c?.message||"RTC start failed"}),ce()}}function ue(e){let t=document.querySelector(".chatbot-toggler"),n=document.querySelector(".chat-actions"),o=t?t.querySelector("i"):null,c=document.querySelector(".pill-btn");function s(){o&&(o.className=n.classList.contains("show")?"bi bi-x-lg":"bi bi-chat-dots")}t&&t.addEventListener("click",()=>{n.classList.toggle("show")||n.classList.remove("show"),s()});let a=document.getElementById("pill-toggle");a&&a.addEventListener("click",()=>{let r=e.getPage();r&&localStorage.setItem("chatbot_prev_page",r),document.body.classList.remove("show-chatbot"),s()}),c&&c.addEventListener("click",r=>{r.preventDefault();try{window.open("https://share-eu1.hsforms.com/2U7ipluaKRqy_okiu5ErdUA2es2nu","_blank","noopener,noreferrer")}catch{window.location.href="https://share-eu1.hsforms.com/2U7ipluaKRqy_okiu5ErdUA2es2nu"}});let i=document.getElementById("action-chat"),p=document.getElementById("action-call");i&&i.addEventListener("click",()=>{n.classList.remove("show"),document.body.classList.add("show-chatbot"),s();let r=localStorage.getItem("chatbot_current_page"),d=localStorage.getItem("chatbot_user_name");e.setPage(r==="chat"||d?"chat":"welcome")}),p&&p.addEventListener("click",()=>{n.classList.remove("show"),document.body.classList.add("show-chatbot"),s(),e.setPage("call-welcome")})}function B(e,t){let n=document.querySelector(".chatbox");if(!n)return;let o=document.createElement("li");if(t==="typing")o.className="chat typing",o.innerHTML='<i class="bi bi-robot"></i><p class="typing"><span></span><span></span><span></span></p>';else{let c=t==="error"?"error":t;o.className=`chat ${c}`,c==="incoming"?o.innerHTML=`<i class="bi bi-robot"></i><div class="bubble">${Te(e)}</div>`:c==="error"?o.innerHTML=`<i class="bi bi-robot"></i><p>${Z(e).replace(/\n/g,"<br>")}</p>`:o.innerHTML=`<p>${Z(e).replace(/\n/g,"<br>")}</p>`}return n.appendChild(o),n.scrollTop=n.scrollHeight,o}function Z(e){return(e||"").replace(/[&<>]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;"})[t])}function Te(e){let n=Z(e||"").split(`
`),o="",c=!1,s=null;function a(){s&&(o+=`</${s}>`,s=null)}function i(p){return p.replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>")}for(let p=0;p<n.length;p++){let r=n[p];if(r.trim().startsWith("```")){c?(c=!1,o+="</code></pre>"):(a(),c=!0,o+="<pre><code>");continue}if(c){o+=r+`
`;continue}if(/^###\s+/.test(r)){a(),o+="<h3>"+r.replace(/^###\s+/,"")+"</h3>";continue}if(/^##\s+/.test(r)){a(),o+="<h2>"+r.replace(/^##\s+/,"")+"</h2>";continue}if(/^#\s+/.test(r)){a(),o+="<h1>"+r.replace(/^#\s+/,"")+"</h1>";continue}if(/^\s*\d+\.\s+/.test(r)){let d=r.replace(/^\s*\d+\.\s+/,"");s!=="ol"&&(a(),s="ol",o+="<ol>"),o+="<li>"+i(d)+"</li>";continue}if(/^\s*[-*]\s+/.test(r)){let d=r.replace(/^\s*[-*]\s+/,"");s!=="ul"&&(a(),s="ul",o+="<ul>"),o+="<li>"+i(d)+"</li>";continue}if(r.trim()===""){a();continue}a(),o+="<p>"+i(r)+"</p>"}return a(),o}function me(e){return`chatbot_history_${e||"default"}`}function se(e){try{return JSON.parse(localStorage.getItem(me(e)))||[]}catch{return[]}}function ie(e,t){try{localStorage.setItem(me(e),JSON.stringify(t))}catch{}}function le(e){e.forEach(t=>B(t.text,t.type))}var de="https://api.zentai.cloud/api/v1";function ge(e,t){let n=e.getPage(),o=t&&t.bot,c=t&&t.chatbotID,s=o&&o.welcome_message||"Hello! How can I help you today?";if(n==="welcome"){let a=document.getElementById("start-conversation");a&&a.addEventListener("click",()=>e.setPage("prechat"));let i=document.getElementById("footer-chat");i&&i.addEventListener("click",()=>e.setPage("prechat"));let p=document.getElementById("footer-call");p&&p.addEventListener("click",()=>e.setPage("call-welcome"))}else if(n==="call-welcome"){let a=document.getElementById("call-welcome-back"),i=document.getElementById("call-welcome-start");a&&a.addEventListener("click",()=>e.setPage("welcome")),i&&i.addEventListener("click",()=>e.setPage("precall"));let p=document.getElementById("footer-call"),r=document.getElementById("footer-chat");p&&p.classList.add("active"),r&&r.classList.remove("active"),r&&r.addEventListener("click",()=>e.setPage("prechat"))}else if(n==="prechat"){let h=function(l){return/.+@.+\..+/.test((l||"").trim())},x=function(){let l=(p&&p.value||"").trim(),m=(r&&r.value||"").trim(),g=d&&d.value||"",w=(u&&u.value||"").trim(),y=(_&&_.value||"").trim(),E=w.length>=7,v=!!l&&h(m)&&!!g&&E&&!!y;i&&(i.disabled=!v)},a=document.getElementById("prechat-form");j("country-code","+234");let i=document.getElementById("start-chat-btn"),p=document.getElementById("prechat-name"),r=document.getElementById("prechat-email"),d=document.getElementById("country-code"),u=document.getElementById("prechat-phone"),_=document.getElementById("prechat-question");p&&p.addEventListener("input",x),r&&r.addEventListener("input",x),d&&d.addEventListener("change",x),u&&u.addEventListener("input",x),_&&_.addEventListener("input",x),a&&a.addEventListener("submit",l=>{l&&l.preventDefault&&l.preventDefault();let m=(document.getElementById("prechat-name").value||"Guest").trim(),g=(document.getElementById("prechat-email").value||"None").trim(),w=document.getElementById("country-code").value||"",y=(document.getElementById("prechat-phone").value||"").trim(),E=(document.getElementById("prechat-question").value||"").trim();localStorage.setItem("chatbot_user_name",m),localStorage.setItem("chatbot_user_email",g),localStorage.setItem("chatbot_user_phone",`${w}${y}`),localStorage.setItem("chatbot_user_question",E),e.setPage("chat")}),i&&i.addEventListener("click",()=>{let l=document.getElementById("prechat-form");i&&i.disabled||l&&l.dispatchEvent(new Event("submit"))});let k=document.getElementById("prechat-back-btn");k&&k.addEventListener("click",()=>e.setPage("welcome"));let f=document.getElementById("footer-chat"),b=document.getElementById("footer-call");f&&f.addEventListener("click",()=>e.setPage("prechat")),b&&b.addEventListener("click",()=>e.setPage("precall")),f&&f.classList.add("active"),b&&b.classList.remove("active")}else if(n==="chat"){let x=function(){let l=localStorage.getItem("chatbot_session_id");return l||(l="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(m){let g=Math.random()*16|0;return(m==="x"?g:g&3|8).toString(16)}),localStorage.setItem("chatbot_session_id",l)),l},k=function(l,m){m!=="error"&&(d.push({text:l,type:m,ts:Date.now()}),ie(c,d))},a=localStorage.getItem("chatbot_user_name")||"Guest",i=localStorage.getItem("chatbot_user_email")||"",p=localStorage.getItem("chatbot_user_phone")||"",r=localStorage.getItem("chatbot_user_question")||"",d=se(c);d.length>0?le(d):B(s,"incoming");let u=document.getElementById("chat-input"),_=document.getElementById("send-btn"),h=document.getElementById("attach-btn");async function f(l){B(l,"outgoing"),k(l,"outgoing");let m=document.querySelector(".chatbox li:last-child");if(m&&m.classList.contains("outgoing")){let y=document.createElement("span");y.className="sender-tag",y.textContent=a||"You",m.appendChild(y)}let g=pe(),w={session_id:x(),message:l,user_email:i??"Guest",user_name:a??"Guest",user_phone:p};try{if(!c)throw new Error("Missing chatbotID");let y=await fetch(`${de}/chatbots/widget/${encodeURIComponent(c)}/chat/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(w)}),E=y.ok,v=E?await y.json():null;if(g(),!E)throw new Error("Request failed");let S=v&&v.bot_message&&v.bot_message.message||v?.data?.response||v?.response||v?.message||"Okay.";B(S,"incoming"),k(S,"incoming")}catch{g(),B("Could not process your request at the moment. Please try again.","error")}}r&&d.length===0&&f(r),_&&_.addEventListener("click",()=>{let l=(u&&u.value||"").trim();l&&(u.value="",f(l))}),u&&u.addEventListener("keydown",l=>{if(l.key==="Enter"&&!l.shiftKey){l.preventDefault();let m=(u&&u.value||"").trim();if(!m)return;u.value="",f(m)}}),h&&h.addEventListener("click",()=>{});let b=document.getElementById("chat-back-btn");b&&b.addEventListener("click",()=>e.setPage("prechat"))}else{if(n==="call-welcome"||n==="precall"||n==="call"||n==="postcall-transfer")return re(e,t);if(n==="prechat"){let a=document.getElementById("prechat-form");a&&a.addEventListener("submit",()=>{let u=(document.getElementById("prechat-name").value||"").trim(),_=(document.getElementById("prechat-email").value||"").trim(),h=document.getElementById("country-code").value||"",x=(document.getElementById("prechat-phone").value||"").trim(),k=(document.getElementById("prechat-question").value||"").trim();localStorage.setItem("chatbot_user_name",u??"Guest"),localStorage.setItem("chatbot_user_email",_),localStorage.setItem("chatbot_user_phone",`${h}${x}`),localStorage.setItem("chatbot_user_question",k),e.setPage("chat")});let i=document.getElementById("start-chat-btn");i&&i.addEventListener("click",()=>{let u=document.getElementById("prechat-form");u&&u.dispatchEvent(new Event("submit"))});let p=document.getElementById("prechat-back-btn");p&&p.addEventListener("click",()=>e.setPage("welcome"));let r=document.getElementById("footer-chat"),d=document.getElementById("footer-call");r&&r.addEventListener("click",()=>e.setPage("prechat")),d&&d.addEventListener("click",()=>e.setPage("precall")),r&&r.classList.add("active"),d&&d.classList.remove("active")}else if(n==="chat"){let x=function(){let l=localStorage.getItem("chatbot_session_id");return l||(l="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(m){let g=Math.random()*16|0;return(m==="x"?g:g&3|8).toString(16)}),localStorage.setItem("chatbot_session_id",l)),l},k=function(l,m){m!=="error"&&(d.push({text:l,type:m,ts:Date.now()}),ie(c,d))},a=localStorage.getItem("chatbot_user_name")||"Guest",i=localStorage.getItem("chatbot_user_email")||"",p=localStorage.getItem("chatbot_user_phone")||"",r=localStorage.getItem("chatbot_user_question")||"",d=se(c);d.length>0?le(d):B(s,"incoming");let u=document.getElementById("chat-input"),_=document.getElementById("send-btn"),h=document.getElementById("attach-btn");async function f(l){B(l,"outgoing"),k(l,"outgoing");let m=document.querySelector(".chatbox li:last-child");if(m&&m.classList.contains("outgoing")){let y=document.createElement("span");y.className="sender-tag",y.textContent=a||"You",m.appendChild(y)}let g=pe(),w={session_id:x(),message:l,user_email:i,user_name:a,user_phone:p};try{if(!c)throw new Error("Missing chatbotID");let y=await fetch(`${de}/chatbots/widget/${encodeURIComponent(c)}/chat/`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(w)}),E=y.ok,v=E?await y.json():null;if(g(),!E)throw new Error("Request failed");let S=v&&v.bot_message&&v.bot_message.message||v?.data?.response||v?.response||v?.message||"Okay.";B(S,"incoming"),k(S,"incoming")}catch{g(),B("Could not process your request at the moment. Please try again.","error")}}r&&d.length===0&&f(r),_&&_.addEventListener("click",()=>{let l=(u&&u.value||"").trim();l&&(u.value="",f(l))}),u&&u.addEventListener("keydown",l=>{if(l.key==="Enter"&&!l.shiftKey){l.preventDefault();let m=(u&&u.value||"").trim();if(!m)return;u.value="",f(m)}}),h&&h.addEventListener("click",()=>{});let b=document.getElementById("chat-back-btn");b&&b.addEventListener("click",()=>e.setPage("prechat"))}else if(n==="call-welcome"){let a=document.getElementById("call-back"),i=document.getElementById("call-end");a&&a.addEventListener("click",()=>e.setPage("call-welcome")),i&&i.addEventListener("click",()=>e.setPage("call-welcome"))}}}function pe(){let e=B("","typing");return function(){e&&e.parentNode&&e.parentNode.removeChild(e)}}window.addEventListener("chatbot-call-status",e=>{});window.addEventListener("chatbot-call-status",e=>{let t=e?.detail?.status||"",n=document.getElementById("call-end"),o=t==="connecting";n&&(n.classList.toggle("loading",o),n.disabled=o)});var Me="Zentrova",ze="https://api.zentai.cloud/api/v1";function Ne(){let e=document.createElement("button");e.className="chatbot-toggler",e.setAttribute("aria-label","Open chat"),e.innerHTML='<i class="bi bi-chat-dots"></i>';let t=document.createElement("div");t.className="chat-actions",t.innerHTML=`
  ...
  `;let o=document.createElement("div");o.className="chatbot",o.innerHTML='<div id="chatbot-content"></div>',document.body.appendChild(e),document.body.appendChild(t),document.body.appendChild(n),document.body.appendChild(o);let c=o.querySelector("#chatbot-content");return{toggle:e,actions:t,pill:n,shell:o,contentRoot:c}}function $e(e){let t=document.querySelector(".chatbot-toggler"),n=document.querySelector(".chat-actions"),o=document.querySelector(".chatbot"),c=document.querySelector(".powered-pill"),s=e==="bottom-left"?"unset":"24px",a=e==="bottom-left"?"24px":"unset";[t,n,o,c].forEach(i=>{i&&(i.style.right=s,i.style.left=a)})}async function De(e){let t=await fetch(`${ze}/chatbots/widget/${encodeURIComponent(e)}/`,{method:"GET"});if(!t.ok)throw new Error("Failed to load bot configuration");return t.json()}async function Re({agentId:e,position:t,brandColor:n,brandName:o}={}){let c=e;try{let g=localStorage.getItem("chatbot_id");g&&c&&g!==c&&localStorage.clear()}catch{}ee(),te();let s=null;try{s=c?await De(c):null}catch{}let a=s&&s.brand_color||n||"#5b2a86",i=s&&s.widget_settings&&s.widget_settings.position||t||"bottom-right",p=!!(s&&s.widget_settings&&s.widget_settings.auto_open),r=s&&(s.name||s.organization_name)||o||Me,d=s&&s.welcome_message||"Hello! How can I help you today?";try{localStorage.setItem("chatbot_id",c||""),localStorage.setItem("chatbot_bot_voice",s&&s.voice||"alloy"),localStorage.setItem("chatbot_bot_language",s&&s.language||"English"),localStorage.setItem("chatbot_bot_welcome_message",d),localStorage.setItem("chatbot_transfer_phone_number",s&&s?.transfer_phone_number||"")}catch{}oe(a);let{toggle:u,actions:_,pill:h,shell:x,contentRoot:k}=Ne();$e(i);let f=ne(r),b={chatbotID:c,bot:s,brandColor:a,position:i,brandName:r,welcomeMessage:d},l=new q(k,f,()=>ge(l,b));(function(){let w=localStorage.getItem("chatbot_session_id");w||(w="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(y){let E=Math.random()*16|0;return(y==="x"?E:E&3|8).toString(16)}),localStorage.setItem("chatbot_session_id",w))})();let m=localStorage.getItem("chatbot_current_page");m&&(l.currentPage=m),ue(l),l.render()}function fe({agentId:e,position:t="bottom-right",brandColor:n="#5b2a86",brandName:o}={}){Re({agentId:e,position:t,brandColor:n,brandName:o})}var he={init:fe};typeof window<"u"&&(window.ChatbotSDK=he);return _e(Oe);})();
